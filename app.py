...
def api_userinfo():
    ...
    try:
        ...
        res = db_query(f"SELECT username, bar_name, qr_data FROM {USERS_TABLE} WHERE user_id=?", (user_id,), fetch=True)
        if res:
            username, bar_name, qr_data = res[0]
            return jsonify(ok=True, username=username, bar_name=bar_name, qr_data=qr_data)
